...
- [x] React Native + Expoプロジェクト作成
- [x] Node.js + Expressバックエンド構築
- [x] 必要なパッケージのインストール
- [x] Gitリポジトリセットアップ
- [x] 環境変数設定（.env）
- [x] 基本的なフォルダ構造作成
...
- [x] expo-avによる録音機能実装
- [x] 録音ファイルの一時保存
- [x] 録音状態の表示（録音中インジケーター）
- [x] エラーハンドリング（権限拒否、録音失敗）
...
#### 3.1 実装タスク
- [x] 音声ファイルアップロードエンドポイント
- [x] OpenAI Whisper APIによる文字起こし
- [x] OpenAI GPT APIによるAI整理
- [x] エラーハンドリング（API失敗、タイムアウト）
- [x] CORS設定
- [x] 環境変数管理（APIキー）
...
#### 4.1 実装タスク
- [x] 録音終了後、バックエンドへファイル送信
- [x] ローディング表示（AI処理中）
- [x] API通信エラーハンドリング
- [x] レスポンスデータの表示
- [x] AsyncStorageへの保存（任意）
